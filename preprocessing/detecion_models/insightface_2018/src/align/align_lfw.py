...
def main(args):
    ...
    if args.num_partition > 1:
      dataset_split = np.array_split(dataset, args.num_partition)
      dataset = list(dataset_split[args.partition_idx])
    
    print('dataset size', len(dataset), 'partition: {} / {}'.format(args.partition_idx, args.num_partition))
    sleep(5)
...
def parse_arguments(argv):
    ...
    parser.add_argument('--input-dir', type=str, default='/data/data/faces/lfw')
    parser.add_argument('--output-dir', type=str, default='/data/data/faces/lfw_insightface_2018_align')
    parser.add_argument('--image-size', type=str, help='Image size (height, width) in pixels.', default='112,112')
    parser.add_argument('--num_partition', type=int, default=8)
